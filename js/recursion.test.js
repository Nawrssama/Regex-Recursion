...
function division(number, dividedBy) {
    ...
    // Base case: if the dividedBy is zero, return Infinity
    if (dividedBy === 0) {
        return 0;
    }

    // Base case: if the number is less than the dividedBy, return 0
    if (number < dividedBy) {
        return 0;
    }

    // Recursive case: subtract the dividedBy from the number, and recursively call the function
...
function pow(x, n) {
  ...
  // Recursive case: call the function with the base and exponent reduced by 1, and multiply the result by the base
  return x * pow(x, n - 1);
}
